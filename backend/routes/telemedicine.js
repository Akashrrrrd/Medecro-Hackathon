...
router.get('/', async (req, res) => {
  try {
    const sessions = await Telemedicine.find().populate('patientId').populate('doctorId');
    res.json(sessions);
  } catch (err) {
...
router.get('/:id', async (req, res) => {
  try {
    const session = await Telemedicine.findById(req.params.id).populate('patientId').populate('doctorId');
    res.json(session);
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
...
router.patch('/:id', async (req, res) => {
  try {
    const updatedSession = await Telemedicine.findByIdAndUpdate(req.params.id, req.body, { new: true });
